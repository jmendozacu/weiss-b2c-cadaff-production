...
register_post_type('blocks',
	array('label' => 'Blocks',
			'description' => 'Create blocks that can be used in posts, pages and widgets.',
			'public' => true,
			'show_ui' => true,
			'show_in_menu' => true,
...
function my_edit_blocks_columns( $columns ) {
	...
	$columns = array(
		...
		'title' => __( 'Title', 'trego' ),
		'shortcode' => __( 'Shortcode', 'trego' ),
		'date' => __( 'Date', 'trego' )
	);
...
function my_manage_blocks_columns( $column, $post_id ) {
	...
	switch( $column ) {
		case 'shortcode' :
			echo '<span style="background:#EEEEEE;font-weight:bold;"> [block id="'.$slug.'"] </span>';
			break;
	}
}
...
function block_shortcode($atts, $content = null) {	
	...
	if($post_id){
		...
		if (current_user_can('edit_posts') && !is_page_template('page-home.php') && !is_page_template('page-one-template.php') && !is_page_template('page-portfolio-template.php')) {
			$edit_link = get_edit_post_link( $post_id );
			$html = '<div class="edit_block"><a class="edit-link" href="'.$edit_link.'" title="Edit Block">Edit Block</a>'.$html.'</div>';
		}
		...
		$html = '<p><mark>EF Block <b>"'.$id.'"</b> not found! Wrong ID?</mark></p>';	
