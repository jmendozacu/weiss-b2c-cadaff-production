...
class Pektsekye_OptionExtended_Model_Mysql4_Template_Option extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    {
      $select = $this->_getReadAdapter()->select()
          ->from($this->getTable('optionextended/template_value'), 'value_id')
          ->where('option_id IN (?)', $ids);
      $valueIds = $this->_getReadAdapter()->fetchCol($select);

     Mage::getResourceModel('optionextended/template_value')->deleteValuesWithChidrenUpdate((int) $templateId, $valueIds);
