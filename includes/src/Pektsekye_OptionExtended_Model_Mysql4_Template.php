...
class Pektsekye_OptionExtended_Model_Mysql4_Template extends Mage_Core_Model_Mysql4_Abstract
{
  ...
  public function getTemplateOptionsCollection($productId, $storeId)
  {
      ...
      $lastOptionId = (int) $this->_getReadAdapter()->fetchOne("SELECT `option_id` FROM `{$this->getTable('catalog/product_option')}` ORDER BY `option_id` DESC LIMIT 1 ");
      ...
      $optionRows = $this->_getReadAdapter()->fetchAll($this->_getReadAdapter()->select()
          ...
          array('*','id' => new Zend_Db_Expr("option_table.option_id + {$lastOptionId}"), 'option_id' => new Zend_Db_Expr("option_table.option_id + {$lastOptionId}")))            
      ...
      $lastOptionTypeId = (int) $this->_getReadAdapter()->fetchOne("SELECT `option_type_id` FROM `{$this->getTable('catalog/product_option_type_value')}` ORDER BY `option_type_id` DESC LIMIT 1 ");
      ...
      $valueRows = $this->_getReadAdapter()->fetchAll($this->_getReadAdapter()->select()
          ...
          ->join(array('value_table'=>$valueTable),
            "value_table.option_id=option_table.option_id ",
             array('*', 'option_id' => new Zend_Db_Expr("value_table.option_id + {$lastOptionId}"), 'id' => new Zend_Db_Expr("value_table.value_id + {$lastOptionTypeId}"), 'option_type_id' => new Zend_Db_Expr("value_table.value_id + {$lastOptionTypeId}")))  
