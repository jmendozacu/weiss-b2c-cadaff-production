...
class Trego_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if(isset($_FILES['image']['name']) and (file_exists($_FILES['image']['tmp_name']))) {
                try {
                    ...
                    $path = Mage::getBaseDir('media') . DS . 'trego' . DS . 'blog' . DS;
                               
                    $uploader->save($path, $_FILES['image']['name']);

                    $data['image'] = 'trego/blog/' . preg_replace('/[^a-z0-9_\\-\\.]+/i', '_',$_FILES['image']['name']);
                }catch(Exception $e) {
