...
abstract class Trego_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = Trego_Blog_Helper_Data::CATEGORY_URI_PARAM;
    
    protected static $_postUriParam = Trego_Blog_Helper_Data::POST_URI_PARAM;
    
    protected static $_tagUriParam = Trego_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    {        
        ...
        foreach ($collection as $item) {
            ...
            if ($shortContent) {
                ...
                $strManager = new Trego_Blog_Helper_Substring(array('input' => self::$_helper->filterWYS($item->getPostContent())));
    ...
    {
        if (self::$_helper->conf(Trego_Blog_Helper_Data::XML_BOOKMARKS)) {
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('trego_blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {      
        return '<a class="trego-blog-read-more" href="'.$item->getAddress().'">'.$this->__('Read More').'</a>';
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == Trego_Blog_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            ...
            $collection = Mage::getModel('blog/blog')->getCollection()
                    ->addPresentFilter()
                    ->addEnableFilter(Trego_Blog_Model_Status::STATUS_ENABLED)
                    ->addStoreFilter()
                    ->joinComments()
