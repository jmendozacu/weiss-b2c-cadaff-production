...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        // this.syncShippingBilling = false;
        this.syncBillingShipping = false;
        this.method = '';
        this.payment = '';
        this.loadWaiting = false;
        this.steps = ['login', 'shipping', 'billing', 'shipping_method', 'payment', 'review'];
...
Billing.prototype = {
    ...
    newAddress: function(isNew){
		...
		billing.setSameAsShipping(false);
    },
    ...
    resetSelectedAddress: function(){
        ...
        if (selectElement) {
			...
			for (var elemIndex in arrElements) {
				if (arrElements[elemIndex].id && arrElements[elemIndex].id != 'billing-address-select'  && arrElements[elemIndex].id != 'billing:save_in_address_book' && arrElements[elemIndex].id != 'billing:country_id') {
    ...
    fillForm: function(transport){
        ...
        arrElements = Form.getElements(this.form);
        for (var elemIndex in arrElements) {
            if (arrElements[elemIndex].id && arrElements[elemIndex].id != 'billing-address-select') {
                var fieldName = arrElements[elemIndex].id.replace(/^billing:/, '');
...
Shipping.prototype = {
    ...
    resetSelectedAddress: function(){
        ...
        if (selectElement) {
			...
			arrElements = Form.getElements(this.form);
			for (var elemIndex in arrElements) {
				if (arrElements[elemIndex].id && arrElements[elemIndex].id != 'shipping-address-select' && arrElements[elemIndex].id != 'shipping:use_for_billing_yes' && arrElements[elemIndex].id != 'shipping:use_for_billing_no' && arrElements[elemIndex].id != 'shipping:save_in_address_book' && arrElements[elemIndex].id != 'shipping:country_id') {
					var fieldName = arrElements[elemIndex].id.replace(/^shipping:/, '');
					arrElements[elemIndex].value = '';
				}
			}
        }
		jQuery('input[name="shipping[use_for_billing]"]').prop('checked', false);
		jQuery('#billing\\:same_as_shipping').prop('checked', false);
    },
    ...
    fillForm: function(transport){
        ...
        arrElements = Form.getElements(this.form);
        for (var elemIndex in arrElements) {
            if (arrElements[elemIndex].id && arrElements[elemIndex].id != 'shipping-address-select') {
                var fieldName = arrElements[elemIndex].id.replace(/^shipping:/, '');
                arrElements[elemIndex].value = elementValues[fieldName] ? elementValues[fieldName] : '';
...
ShippingMethod.prototype = {
    ...
    validate: function() {
        ...
        alert(Translator.translate('Veuillez sélectionner un mode de transport').stripTags());
        return false;
    },
    ...
    nextStep: function(transport){
        ...
        payment.initWhatIsCvvListeners();
		if(jQuery('input[name="shipping_method"]:checked').val().search('owebiashipping3_livraison') === 0){
			jQuery('.dt_cashondelivery').show();
		}
...
Payment.prototype = {
    ...
    changeVisible: function(method, mode) {
		...
		jQuery('.subactive').removeClass('subactive');
        ...
        [block + '_before', block, block + '_after'].each(function(el) {
            ...
            if (element) {
				...
				if(!mode){
					jQuery(element).closest('dd').addClass('subactive');
					jQuery(element).closest('dd').prev().addClass('active');
				}
